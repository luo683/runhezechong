...
export default async function ProductDetailPage({
  ...
  if (!product) {
    return (
      <div className="max-w-6xl mx-auto px-10 py-32 text-center">
        ...
        <Link href="/products" className="text-warm-accent text-sm">
          ← 返回产品列表
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto px-10 py-16">
      <Link
        href="/products"
        className="text-xs text-warm-text-dim hover:text-warm-accent transition-colors"
      >
        ← 返回产品列表
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mt-8">
        ...
        <div>
          ...
          <h1 className="font-serif italic text-2xl lg:text-4xl text-warm-text mt-3 mb-4">
            {product.name}
          </h1>
          <p className="text-2xl font-medium text-warm-accent mb-6">
            ¥{product.price}
          </p>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {product.effects.map((e) => (
                <span
                  key={e}
                  className="text-xs px-3 py-1 bg-warm-border/30 rounded-full text-warm-text-dim"
                >
                  {e}
                </span>
              ))}
